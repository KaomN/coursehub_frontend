<template>
    ...
    <div class="course-view-container">
        <h2 class="pb-4">My Subscribed Courses</h2>
        <div class="cards-container">
            <div v-if="items.length == 0 && fetched">
                <div>No Subscribed Courses</div>
            </div>
            <div v-else>
                    ...
                    >
                        ...
                        <template #footer>
                            <div class="custom-card custom-card-footer">
                                <BButton
                                    variant="danger"
                                    @click="unsubscribe(item.sys_id, rowIndex, itemIndex)"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions(['actionGetCourses', 'actionUnsubscribe', 'actionGetSubscribed']),
            async getCourses() {
                this.isLoading = true
                const courseIds = this.subscribedCourses.map(item => item.course.value).join(', ')
                const params = {
                    sysparm_fields: 'title,description,duration,type,sys_id',
                    sysparm_query: `sys_idIN${courseIds}`
                }
            ...
            async unsubscribe(courseId, cardRowIndex, cardIndex) {
                this.btnLoadingState[cardRowIndex][cardIndex] = true
 
                const subscription = this.subscribedCourses.find(course => course.course.value === courseId)
                const res = await this.actionUnsubscribe(subscription.sys_id)
                if (res.success) {
